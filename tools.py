...
def count_episodes(directory):
    # 电视剧有多少集，每集有多少分钟，这些东西都是放在episodes的文件夹下面的
...
def static_scan(fn, inputs, start, reverse=False):
    ...
    # print("400 tools:: ", tf.nest.flatten(inputs)[0])
    # print("400 tools:: ", len(tf.nest.flatten(inputs)[0]))
    # indices = range(len(tf.nest.flatten(inputs)[0])) # Bug? Yong Lee
    indices = range(tf.nest.flatten(inputs)[0].shape[0])  # Bug? Yong Lee
    if reverse:
